...
%% Create cross dataset for combined case 
...
%% Fit coefficients for combined case - $F_z$ = $F_z0$ , $\gamma=0$ prova 
% extract data with variable load
[TDataComb, ~] = intersect_table_data( tyre_data_LONG, GAMMA_0, FZ_700);

% Fit the coeffs {'rBy1', 'rBy2', 'rBy3', 'rCy1', 'rHy1', 'rVy1', 'rVy4', 'rVy5', 'rVy6'};

% Guess values for parameters to be optimised
% P0 = [  1,   2,   1,  0,   0,   1,   0,   1,    0]; 
...
zeros_vec = zeros(size(TDataComb.SA));
ones_vec  = ones(size(TDataComb.SA));
